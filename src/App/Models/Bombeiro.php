...
Class Bombeiro {
	...
	public function __construct() {
		...
		switch ($this->langTxt) {
		    case 'pt':
		        $this->moduleName = 'bombeiros';
		        break;
		    case 'en':
		        $this->moduleName = 'firefighters';
		        break;
		    default:
		    	$this->moduleName = 'firefighters';
		}
	...
	public function convertIntoMapPoints($rows) {
		...
		foreach($rows as $row) {
			...
			$mapPoints[] = array(
				...
				"icon" => array(
					"iconUrl" => SITE_CONFIGS['info']['baseURL'].'/assets/images/map/icons/fireman.png',
					"iconSize" => [32, 32],
				),
				"address" => $row['address'],
				"url" => $row['url'],
	...
	public function getStationsList($options = null) {
		$sql = "SELECT i.*, i_lang.title, i_lang.subtitle, i_lang.description, ST_Y(i.gps) AS gps_lat, ST_X(i.gps) AS gps_lon";
		$sqlCount = "SELECT COUNT(*) AS total";

		$sqlAppend = " FROM md_bombeiros AS i
				LEFT JOIN md_bombeiros_lang AS i_lang
	...
	public function stationsPageInfo(int $id) {
		$sql = "SELECT b.*, b_lang.title, b_lang.subtitle, b_lang.description, ST_Y(b.gps) AS gps_lat, ST_X(b.gps) AS gps_lon
				FROM md_bombeiros AS b
				LEFT JOIN md_bombeiros_lang AS b_lang
					ON b_lang.parent = b.id
					AND b_lang.id_lang = {$this->lang}
				WHERE 1
				AND b.active = 1
					AND b.visible = 1
					AND b.dt_delete IS NULL
					AND b.id = ". (int) \DB::escape_string($id);
		...
		if (!empty($row['id_freguesia'])) {
			$output['sidebar']['municipality_stations'] = $this->getStationsList(array(
				"id_not" => $row['id'],
	...
	public function breadcrumbs() {
		$breadcrumbs = array(
			...
			array(
				...
				"url" => SITE_CONFIGS['info']['baseURL']."/".$this->langTxt."/".\Lang\Dictionary::get('urlparam_firefighters'),
			),
		);

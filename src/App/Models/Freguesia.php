...
Class Freguesia {
	...
	public function __construct() {
		...
		switch ($this->langTxt) {
		    case 'pt':
		        $this->moduleName = 'freguesia';
		        break;
		    case 'en':
		        $this->moduleName = 'parish';
		        break;
		    default:
		    	$this->moduleName = 'parish';
		}
	...
	public function overallPageInfo(int $id, $tab = null) {
		$sql = "SELECT f.*, f_lang.title, f_lang.subtitle, f_lang.description, ST_Y(f.gps) AS gps_lat, ST_X(f.gps) AS gps_lon, f.gps_z, f.parent AS id_concelho, c.parent AS id_distrito
				FROM md_localidades AS f
				LEFT JOIN md_localidades_lang AS f_lang
					ON f_lang.parent = f.id
					AND f_lang.id_lang = {$this->lang}
		...
		$sql .= " AND f.active = 1
					AND f.visible = 1
					AND f.dt_delete IS NULL";
		*/
		$sql .= " AND f.id_type = 3
					AND f.id = ". (int) \DB::escape_string($id);
		...
		$localidade = array(
			...
			"gps" => array(
				"lat" => $row['gps_lat'],
				"lon" => $row['gps_lon'],
				"z" => $row['gps_z'] ? $row['gps_z'] : 15,
			),
		);

		$concelhoInfo = $concelhoModel->itemsBasicInfo(array("id" => $id_concelho));
		$localidade['concelho'] = array(
			"id" => $id_concelho,
			"title" => $concelhoInfo[$id_concelho]['title'],
			"url" => $concelhoInfo[$id_concelho]['url'],
		);
		...
		$output = array(
			...
			"breadcrumbs" => $this->breadcrumbs($distritoInfo[$id_distrito], $concelhoInfo[$id_concelho], $row, $title, $tab),
		);

		return $output;
	}

	protected function breadcrumbs($distrito, $concelho, $row, $title, $tab = null) {
		$breadcrumbs = array(
			array(

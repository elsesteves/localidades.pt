<?php
	$searchFormURL = $site['baseURL'] .'/'. $pageLang .'/'. \Lang\Dictionary::get('urlparam_events') .'/'. \Lang\Dictionary::get('urlparam_search');
?>
<form method="GET" action="<?= $searchFormURL ?>">
	<div class="row search-form">

		<?php 
		$formErrors = \Form::getFormErrors('event_search');
		...
		<div class="col-4 mb-2 form-group">
			<label for="types-select"><?= \Lang\Dictionary::get('event_types') ?></label>
			<select class="form-control" id="types-select" name="id_tipo">
				<option value=""><?= \Lang\Dictionary::get('event_type_select') ?></option>
				<?php foreach($types as $type) : ?>
					<option value="<?= $type['id'] ?>" <?php 
					if (exists($_REQUEST['id_tipo']) && $_REQUEST['id_tipo'] == $type['id']) {
						print 'selected';
					}
					?>><?= $type['title'] ?></option>
				<?php endforeach; ?>
			</select>
			<?php if(exists($formErrors['id_tipo']['errors'])) : ?>
				<small class="errors"><?= implode('<br>', $formErrors['id_tipo']['errors']) ?></small>
			<?php endif; ?>
		</div>

		<div class="col-4 mb-2 form-group">
			<label for="event_types"><?= \Lang\Dictionary::get('dt_from').':' ?></label>
			<input type="date" class="form-control" id="dt_from" name="dt_from" value="<?php if (exists($_REQUEST['dt_from'])) { print $_REQUEST['dt_from']; } ?>">
			<?php if(exists($formErrors['dt_from']['errors'])) : ?>
				<small class="errors"><?= implode('<br>', $formErrors['dt_from']['errors']) ?></small>
			<?php endif; ?>
		</div>

		<div class="col-4 mb-2 form-group">
			<label for="event_types"><?= \Lang\Dictionary::get('dt_to').':' ?></label>
			<input type="date" class="form-control" id="dt_to" name="dt_to" value="<?php if (exists($_REQUEST['dt_to'])) { print $_REQUEST['dt_to']; } ?>">
			<?php if(exists($formErrors['dt_to']['errors'])) : ?>
				<small class="errors"><?= implode('<br>', $formErrors['dt_to']['errors']) ?></small>
			<?php endif; ?>
		</div>

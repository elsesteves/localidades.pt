<?php
	$searchFormURL = $site['baseURL'] .'/'. $pageLang .'/'. \Lang\Dictionary::get('urlparam_schools') .'/'. \Lang\Dictionary::get('urlparam_search');
?>
<form method="GET" action="<?= $searchFormURL ?>">
	<div class="row search-form">

		<?php 
		$formErrors = \Form::getFormErrors('school_search');
		...
		<div class="col-4 mb-2 form-group">
			<label for="cycle-select"><?= \Lang\Dictionary::get('school_cycles') ?></label>
			<select class="form-control" id="cycle-select" name="id_ciclo">
				<option value=""><?= \Lang\Dictionary::get('school_cycle_select') ?></option>
				<?php foreach($cycles as $cycle) : ?>
					<option value="<?= $cycle['id'] ?>" <?php 
					if (exists($_REQUEST['id_ciclo']) && $_REQUEST['id_ciclo'] == $cycle['id']) {
						print 'selected';
					}
					?>><?= $cycle['title'] ?></option>
				<?php endforeach; ?>
			</select>
			<?php if(exists($formErrors['id_ciclo']['errors'])) : ?>
				<small class="errors"><?= implode('<br>', $formErrors['id_ciclo']['errors']) ?></small>
			<?php endif; ?>
		</div>
